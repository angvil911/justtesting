...
if python -c "import django" 2>/dev/null; then
    ...
    if grep -q "22.04" /etc/os-release || grep -q "VERSION_ID=\"9" /etc/os-release; then
        wget -q -O /tmp/requirements.txt https://raw.githubusercontent.com/angvil911/justtesting/v2.4.5/requirments.txt
    else
        wget -q -O /tmp/requirements.txt https://raw.githubusercontent.com/angvil911/justtesting/v2.4.5/requirments-old.txt
    fi
