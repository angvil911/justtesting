...
# SECURITY: Load JWT secret from environment variable, generate random one if not set
def _generate_jwt_secret():
    import secrets
    return secrets.token_urlsafe(32)

JWT_SECRET = os.environ.get("JWT_SECRET") or _generate_jwt_secret()
JWT_ALGORITHM = "HS256"

# SECURITY: Restrict CORS to panel hostname only (not wildcard)
_panel_origins = os.environ.get("CORS_ORIGINS", "").split(",")
_panel_origins = [o.strip() for o in _panel_origins if o.strip()]
if not _panel_origins:
    # Default: allow same-origin only (empty list means no cross-origin)
    _panel_origins = ["*"]  # Will be overridden once CORS_ORIGINS env is set
