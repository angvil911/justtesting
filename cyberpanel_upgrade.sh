...
Temp_Value='{"version":"2.3.6","build": 4}'
Panel_Version="2.4"
Panel_Build="5"
...
if [ $CYBERCP_MISSING -eq 1 ]; then
    ...
    if git clone https://github.com/angvil911/justtesting CyberCP_recovery_tmp; then
...
if [[ "$Server_Country" = "CN" ]] ; then
  sed -i 's|git clone https://github.com/angvil911/justtesting|echo git cloned|g' upgrade.py

  Retry_Command "git clone ${Git_Clone_URL}"
    Check_Return "git clone ${Git_Clone_URL}"

  # shellcheck disable=SC2086
  sed -i 's|https://raw.githubusercontent.com/angvil911/justtesting/main/install/litespeed/httpd_config.xml|'${Git_Content_URL}/${Branch_Name}'//install/litespeed/httpd_config.xml|g' upgrade.py
