...
class ClusterManager:
    ...
    LogURL = ""  # Telemetry removed — no external reporting
    UptimeURL = ""  # Telemetry removed — no external reporting
    ...
    def PostStatus(self, message):
        try:
            # Log locally only — external telemetry removed
            if os.path.exists(ProcessUtilities.debugPath):
                logging.writeToFile(message + '[info]')
...
password=%s""" % (rootdbpassword, rootdbpassword)
    ...
    def Uptime(self):
        try:
            # Uptime telemetry removed — log locally only
            if os.path.exists(ProcessUtilities.debugPath):
                logging.writeToFile('Uptime check [local only]')
        except BaseException as msg:
            logging.writeToFile('%s. [31:404]' % (str(msg)))
