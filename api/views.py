...
def remoteTransfer(request):
    try:
        if request.method == "POST":
            ...
            try:
                _ipaddress.ip_address(ipAddress.strip())
            except ValueError:
                return HttpResponse(json.dumps({"transferStatus": 0, 'error_message': "Invalid IP address format."}))
            ...
            if hashPassword.check_password(admin.password, password):
                ...
                execPath = execPath + " remoteTransfer --ipAddress " + ipAddress.strip() + " --dir " + dir + " --accountsToTransfer " + path
...
def FetchRemoteTransferStatus(request):
    try:
        if request.method == "POST":
            ...
            # SECURITY: Validate dir is numeric only to prevent path traversal
            dir_val = str(data['dir'])
            if not dir_val.isdigit():
                return HttpResponse(json.dumps({'fetchStatus': 0, 'error_message': 'Invalid directory format'}))
            dir = "/home/backup/transfer-" + dir_val + "/backup_log"
